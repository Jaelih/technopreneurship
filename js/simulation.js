...
var Simulation = (function () {
  ...
  function buildEastExitArea() {
    ...
    // House A — gable roof, northeast (x=34, z=14)
    box(34, 3, 14, 10, 6, 9, 0xd4a76a);
    addColl(29, 39, 9.5, 18.5, 'lower');
    var rA = new THREE.Mesh(new THREE.BoxGeometry(10.6, 0.4, 9.6), mat(0x2a4a7a));
    rA.position.set(34, 6.2, 14); scene.add(rA);
    var ridgeA = new THREE.Mesh(new THREE.CylinderGeometry(0.001, 5.5, 1.8, 3), mat(0x2a4a7a));
    ridgeA.rotation.x = Math.PI / 2; ridgeA.rotation.z = Math.PI;
    ridgeA.scale.set(1, 9 / 1.8, 1);
    ridgeA.position.set(34, 7.0, 14); scene.add(ridgeA);
  ...
  function buildOutdoorBuildings() {
    ...
    var houses = [
      { x: -18, z: -32, w: 12, h: 7, d: 10, roof: 'gable' },
      { x: -30, z: -45, w: 8,  h: 5, d: 8,  roof: 'hip' },
      { x: -18, z: -55, w: 10, h: 6, d: 9,  roof: 'gable' },
      { x: 20,  z: -28, w: 11, h: 6, d: 8,  roof: 'hip' },
      { x: 28,  z: -42, w: 9,  h: 8, d: 10, roof: 'gable' },  // west of east road
      { x: 22,  z: -58, w: 10, h: 6, d: 8,  roof: 'hip' },
      { x: -11, z: -36, w: 6,  h: 5, d: 6,  roof: 'gable' },
      { x: 11,  z: -66, w: 6,  h: 5, d: 6,  roof: 'hip' },
      { x: -40, z: -68, w: 14, h: 7, d: 12, roof: 'gable' },
      { x: 55,  z: -70, w: 14, h: 7, d: 12, roof: 'gable' },  // east of east road, past NPC
      { x: -25, z: -82, w: 10, h: 6, d: 8,  roof: 'hip' },
      { x: 25,  z: -88, w: 10, h: 6, d: 8,  roof: 'gable' },  // moved south so doesn't block D3 sightline
      { x: -30, z: -38, w: 8,  h: 4, d: 6,  roof: 'hip' },
      { x: -45, z: -90, w: 12, h: 6, d: 10, roof: 'gable' },
      { x: 58,  z: -92, w: 12, h: 6, d: 10, roof: 'hip' },    // east of east road
      { x: 55,  z: -50, w: 10, h: 6, d: 8,  roof: 'hip' },    // east of collapse for neighborhood feel
      { x: 26,  z: -118, w: 9, h: 5, d: 7,  roof: 'gable' }   // house on shelter-approach west side
    ];
    ...
    houses.forEach(function (b, i) {
      ...
      // roof
      if (b.roof === 'gable') {
        var rGeo = new THREE.BoxGeometry(b.w + 0.6, 0.4, b.d + 0.6);
        var rMesh = new THREE.Mesh(rGeo, mat(rc));
        rMesh.position.set(b.x, b.h + 0.2, b.z);
        scene.add(rMesh);
        // pitched ridge
        var ridge = new THREE.Mesh(new THREE.CylinderGeometry(0.001, b.w * 0.55, 1.8, 3), mat(rc));
        ridge.rotation.x = Math.PI / 2;
        ridge.rotation.z = Math.PI;
        ridge.scale.set(1, b.d / 1.8, 1);
        ridge.position.set(b.x, b.h + 1.0, b.z);
        scene.add(ridge);
      } else {
